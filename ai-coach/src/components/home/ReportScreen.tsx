...
export function ReportScreen() {
  ...
  const ensureReportAccessToken = async (): Promise<string> => {
    if (accessToken) return accessToken;

    const { data: sessionData } = await supabase.auth.getSession();
    const existingToken = sessionData?.session?.access_token;
    if (existingToken) {
      setAuth(sessionData.session?.user?.id ?? null, existingToken);
    ...
    if (error) {
      ...
      throw new Error('Unable to start a premium session. Please try again in a moment.');
    }

    const newToken = data.session?.access_token;
    if (!newToken) {
      throw new Error('Unable to authenticate request. Please try again.');
    }

    setAuth(data.user?.id ?? null, newToken);
    return newToken;
  };
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      token = await ensureReportAccessToken();
